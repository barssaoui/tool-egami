...
if os_path.exists('/usr/lib/enigma2/python/Plugins/Extensions/EGAMIClearMem'):
    from Plugins.Extensions.EGAMIClearMem.plugin import SetupMenu
    HAVE_EGAMICLEARMEM = True
else:
    HAVE_EGAMICLEARMEM = False
...
class EgamiMainPanel(Screen):
    ...
    def keyOk(self):
        ...
        elif selected == _('Clear Memory Setup'):
            self.session.open(SetupMenu)
    ...
    def GenerateSettingsMenu(self):
        ...
        if HAVE_EGAMICLEARMEM:
            self.list.append(EgamiMenuEntryComponent('Clear Memory Setup', _('Speed up system by clear memory'), _('Speed up system by clear memory'), 'main/speed'))
