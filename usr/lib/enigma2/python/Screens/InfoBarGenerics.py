...
from enigma import eTimer, eServiceCenter, eDVBServicePMTHandler, iServiceInformation, iPlayableService, eServiceReference, eEPGCache, eActionMap, eDVBVolumecontrol, getDesktop, quitMainloop, ePoint, eDVBDB
...
def ToggleVideo():
    ...
    print '[InfoBarGenerics] ToogleVideo ', mode
    if mode == 'letterbox':
...
class InfoBarUnhandledKey():
    ...
    def actionA(self, key, flag):
        try:
            print '[InfoBarGenerics] KEY: %s %s %s %s %s' % (key,
             flag,
            ...
            try:
                print '[InfoBarGenerics] KEY: %s %s %s %s' % (key,
                 flag,
                 (key_name for key_name, value in KEYIDS.items() if value == key).next(),
                 datetime.now())
            except:
                print '[InfoBarGenerics] KEY: %s %s %s' % (key, flag, datetime.now())
...
class InfoBarShowHide(InfoBarScreenSaver):
    ...
    def __init__(self):
        self['ShowHideActions'] = ActionMap(['InfobarShowHideActions'], {'toggleShow': self.okButtonPressed,
         'hide': self.keyHide}, 1)
    ...
    def _onMove(self, star_position, end_position, step, minstep = 1, timestep = 10):
        self.nextPosition = end_position.y()
        if len(self.onMove) == 0:
            self.instance.startMoveAnimation(star_position, end_position, step, minstep, timestep)
        else:
            self.instance.move(end_position)
            for x in self.onMove:
                x(star_position, end_position, step, minstep, timestep)
...
class InfoBarChannelSelection():
    ...
    def __init__(self):
        ...
        self['ChannelSelectActions'] = HelpableActionMap(self, 'InfobarChannelSelection', {'switchChannelUp': (self.switchChannelUp, _('Open service list and select the previous channel')),
    ...
    def reCallService(self):
        if isinstance(self, InfoBarPiP) and self.pipHandles0Action():
            self.pipDoHandle0Action()
        elif config.usage.panicbutton.getValue():
            self.servicelist.history_tv = []
            self.servicelist.history_radio = []
            self.servicelist.history = self.servicelist.history_tv
            self.servicelist.history_pos = 0
    ...
    def switchChannelUp(self):
        if config.usage.zap_with_arrow_buttons.value:
            self.zapDown()
        elif not config.usage.show_bouquetalways.value:
...
class InfoBarEPG():
    ...
    def InfoPressed(self):
        if isStandardInfoBar(self) or isMoviePlayerInfoBar(self):
            if getBrandOEM() in ('skylake', 'xtrend', 'odin', 'dags', 'gigablue', 'xp'):
                self.openEventView()
            else:
...
class InfoBarSeek():
    ...
    def doSeekRelative(self, pts):
        ...
        setpause = getMachineBuild() in ('vs1500',) and 1
        if self.seekstate == self.SEEK_STATE_EOF:
...
class InfoBarExtensions():
    ...
    def getCCcamInfo(self):
        ...
        for softcam in softcams:
            if 'cccam' in softcam.lower() and config.cccaminfo.showInExtensions.value:
    ...
    def getOScamInfo(self):
        if pathExists('/usr/emu_scripts/'):
            softcams = os.listdir('/usr/emu_scripts/')
        for softcam in softcams:
            if 'oscam' in softcam.lower() and config.oscaminfo.showInExtensions.value:
...
class InfoBarPiP():
    ...
    def showPiP(self):
        ...
        if self.session.pipshown:
            ...
            if info and int(xres) <= 720 or about.getCPUString() == 'BCM7346B2' or about.getCPUString() == 'BCM7425B2' or about.getCPUString() == 'BCM7429B0' or getBoxType() in ('dm900', 'vuultimo4k', 'vuuno4k', 'vusolo4k', 'mutant51', 'ax51', 'sf4008'):
...
class InfoBarInstantRecord():
    ...
    def recordQuestionCallback(self, answer):
        ...
        if answer[1] == 'changeduration':
            ...
            if url:
                import urllib2
                from EGAMI.EGAMI_tools import FileDownloadJob
                from Screens.TaskView import JobView
                try:
...
class InfoBarCueSheetSupport():
    ...
    def __init__(self, actionmap = 'InfobarCueSheetActions'):
        self['CueSheetActions'] = HelpableActionMap(self, actionmap, {'jumpPreviousMark': (self.jumpPreviousMark, _('Jump to the previous marked position')),
